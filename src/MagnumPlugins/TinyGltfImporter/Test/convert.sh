...
# gltf -> glb
for i in animation animation-embedded camera empty image image-embedded image-buffer image-buffer-embedded image-basis image-basis-embedded light material-blinnphong material-metallicroughness material-specularglossiness material-properties mesh-colors mesh-colors-embedded mesh-primitives mesh-primitives-embedded mesh-with-stride mesh mesh-embedded scene scene-nodefault object-transformation texture-default-sampler texture-empty-sampler texture; do
    ./gltf2glb.py ${i}.gltf
done
